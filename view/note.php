<?php
    if (isset($_SESSION['pb'])) {
	      ...
	      if ($_GET['st']==1) {
	        ...
	        if ($conn->query("SELECT*FROM catatan WHERE id_user='$id_user'")->num_rows!==0) {
	            ...
	            while ($get_month=$query_month->fetch_assoc()) {
	              ...
	              if ($cek!==0) {
	                echo "<h4 class='sub-header'><strong>Izin:</strong> $get_user[name_user] - <i>$month</i> </h4>";
        ...
        echo "<h3 class='sub-header'>Izinku <button type='button' onclick=\"window.location.href='tambah_catatan';\" class='btn btn-success'>Buat Izin Lagi</button> </h3>";
	      ...
	      if ($_GET['st']==1) {
	        ...
	        if ($conn->query("SELECT*FROM catatan WHERE id_user='$id_user'")->num_rows!==0) {
	            ...
	            while ($get_month=$query_month->fetch_assoc()) {
	              ...
	              $cek = $query_note->num_rows;
	              if ($cek!==0) {
	                echo "<h4 class='sub-header'>Izin - $month </h4>";
	                echo "<div class='table-responsive'>
	                   <table class='table table-striped'>
	                    <thead>
	                       <tr>
