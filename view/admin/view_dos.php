<h3 class='page-header'>Daftar Dosen</h3>
<?php 
if (isset($_GET['st'])) {
        if ($_GET['st']==1) {
 ...
 ?>
    ...
    <?php 
        if (isset($_GET['id_dsn'])) {
            if ($_GET['id_dsn']!=="") {
                $id_user=$_GET['id_dsn'];
                include './view/admin/edit_dosen.php';
            } else {
...
$q = "SELECT*FROM detail_pb ORDER BY alamat_dsn ASC LIMIT $limitvalue, $limit";
    ...
    if ($r->num_rows!==0) {
            echo "<table class='table table-striped' style='width:70%'>
                    ...
                    while ($get_siswa = $query_siswa->fetch_assoc()) {
                        ...
                        $school = $get_siswa['alamat_dsn'];
                        $no++;
                        echo "<tr>
                                <td>$no</td>
                                <td>$name</td>
                                <td><strong>$school</strong></td>
                                <td><a href='dosen&id_dsn=$id_siswa' title='Edit $name'>Info Lebih Lanjut</a>
                            </tr>";
                    }
                   // $conn->close();
                    echo "</tbody></table>";
