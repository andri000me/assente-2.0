 ...
 ?>
    ...
    <?php 
        if (isset($_GET['id_adm'])) {
            if ($_GET['id_adm']!=="") {
                $id_user=$_GET['id_adm'];
                include './view/admin/edit_admin.php';
            } else {
...
$slice = 9;
$self_server = "./siswa";
$q = "SELECT * FROM detail_admin ORDER BY id_user ASC";
$r = $conn->query($q);
...
$q = "SELECT*FROM detail_admin ORDER BY alamat_adm ASC LIMIT $limitvalue, $limit";
    ...
    if ($r->num_rows!==0) {
            echo "<table class='table table-striped' style='width:70%'>
                    ...
                    while ($get_siswa = $query_siswa->fetch_assoc()) {
                        ...
                        $name = $get_siswa['nama_adm'];
                        $school = $get_siswa['alamat_adm'];
                        $no++;
                        echo "<tr>
                                <td>$no</td>
                                <td>$name</td>
                                <td><strong>$school</strong></td>
                                <td><a href='admin&id_adm=$id_siswa' title='Edit $name'>Edit info</a> &bullet; <a style='cursor:pointer' onclick='hapusSiswa($id_siswa)' >Hapus</a></td>
                            </tr>";
                    }
                   // $conn->close();
                    echo "</tbody></table>";
